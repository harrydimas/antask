...
public class NodeResource {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<NodeDTO> getNode(@PathVariable final String id) {
        return ResponseEntity.ok(nodeService.get(id));
    }

    @PostMapping
    @ApiResponse(responseCode = "201")
    public ResponseEntity<String> createNode(@RequestBody @Valid final NodeDTO nodeDTO) {
        return new ResponseEntity<>(nodeService.create(nodeDTO), HttpStatus.CREATED);
    }

    @PutMapping("/{id}")
    public ResponseEntity<Void> updateNode(@PathVariable final String id, @RequestBody @Valid final NodeDTO nodeDTO) {
    ...
    public ResponseEntity<Void> deleteNode(@PathVariable final String id) {
